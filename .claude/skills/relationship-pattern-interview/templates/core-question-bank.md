...
After the mandatory opening has explained the interview and how answers become evidence, ask:

> Of the interview’s intended outcomes—understanding why patterns repeat, deciding what to change, clarifying the relationship you want, or testing a specific concern—which would make the biggest difference in your real dating life?

Record the answer as the **primary outcome**. If the participant names several outcomes without prioritizing one, do not infer priority from word order. Use one adaptive clarification before Q2:

> If we could optimize this interview for only one of those outcomes, which should lead—and which should be treated as secondary?

Immediately apply the `Outcome routing` table in `SKILL.md`: it determines adaptive follow-up allocation, focal-case selection, checkpoint emphasis, final-artifact priority, and when sufficient evidence permits early stopping. The outcome does not lower evidence standards or authorize confirmation bias.
...
Use an adaptive follow-up only if at least one is true:

- it fills a material gap in chronology;
- it distinguishes two competing explanations;
- it checks safety or consent;
- it gathers a concrete episode supporting a major claim;
- it produces counterevidence;
- it changes the likely action plan.

Do not use a follow-up merely because the topic is interesting.
